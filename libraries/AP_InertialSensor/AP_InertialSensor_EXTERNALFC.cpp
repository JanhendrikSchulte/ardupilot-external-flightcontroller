...
void AP_InertialSensor_SITL::generate_accel()
{
    ...
    for (uint8_t j = 0; j < nsamples; j++) {
        ...
        if (sitl->accel_file_rw == EXTERNALFC::SIM::INSFileMode::INS_FILE_WRITE) {
...
#if AP_SIM_INS_FILE_ENABLED
        if (sitl->gyro_file_rw == EXTERNALFC::SIM::INSFileMode::INS_FILE_WRITE) {
...
#if AP_SIM_INS_FILE_ENABLED
            if (sitl->accel_file_rw == EXTERNALFC::SIM::INSFileMode::INS_FILE_READ
                || sitl->accel_file_rw == EXTERNALFC::SIM::INSFileMode::INS_FILE_READ_STOP_ON_EOF) {
...
#if AP_SIM_INS_FILE_ENABLED
            if (sitl->gyro_file_rw == EXTERNALFC::SIM::INSFileMode::INS_FILE_READ
                || sitl->gyro_file_rw == EXTERNALFC::SIM::INSFileMode::INS_FILE_READ_STOP_ON_EOF) {
                read_gyro_from_file();
            } else
#endif
            generate_gyro();
...
void AP_InertialSensor_SITL::start()
{
    ...
    if (sitl->gyro_file_rw == EXTERNALFC::SIM::INSFileMode::INS_FILE_READ
        || sitl->gyro_file_rw == EXTERNALFC::SIM::INSFileMode::INS_FILE_READ) {
...
void AP_InertialSensor_SITL::read_gyro_from_file()
{
    ...
    if (ret <= 0) {
        if (sitl->gyro_file_rw == EXTERNALFC::SIM::INSFileMode::INS_FILE_READ_STOP_ON_EOF) {
...
void AP_InertialSensor_SITL::read_accel_from_file()
{
    ...
    if (ret <= 0) {
        if (sitl->accel_file_rw == EXTERNALFC::SIM::INSFileMode::INS_FILE_READ_STOP_ON_EOF) {
#if HAL_LOGGING_ENABLED
            //stop logging
            if (AP_Logger::get_singleton()) {
                AP::logger().StopLogging();
