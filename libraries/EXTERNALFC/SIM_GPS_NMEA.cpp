...
void GPS_NMEA::publish(const GPS_Data *d)
{
    ...
    if (_sitl->gps[instance].hdg_enabled == EXTERNALFC::SIM::GPS_HEADING_HDT) {
        nmea_printf("$GPHDT,%.2f,T", d->yaw_deg);
    }
    else if (_sitl->gps[instance].hdg_enabled == EXTERNALFC::SIM::GPS_HEADING_THS) {
        nmea_printf("$GPTHS,%.2f,%c,T", d->yaw_deg, d->have_lock ? 'A' : 'V');
    } else if (_sitl->gps[instance].hdg_enabled == EXTERNALFC::SIM::GPS_HEADING_KSXT) {
        // Unicore support
