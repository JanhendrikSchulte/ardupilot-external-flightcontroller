...
void GPS_UBlox::update_relposned(ubx_nav_relposned &relposned, uint32_t tow_ms, float yaw_deg)
{
    ...
    for (uint8_t i=0; i<ARRAY_SIZE(_sitl->gps); i++) {
        ...
        if (_sitl->gps[i].hdg_enabled != EXTERNALFC::SIM::GPS_HEADING_BASE) {
            continue;
        }
...
void GPS_UBlox::publish(const GPS_Data *d)
{
    ...
    case EXTERNALFC::SIM::GPS_HEADING_NONE:
    case EXTERNALFC::SIM::GPS_HEADING_BASE:
        break;
    case EXTERNALFC::SIM::GPS_HEADING_THS:
    case EXTERNALFC::SIM::GPS_HEADING_KSXT:
    case EXTERNALFC::SIM::GPS_HEADING_HDT:
    ...
    if (_sitl->gps[instance].hdg_enabled > EXTERNALFC::SIM::GPS_HEADING_NONE) {
