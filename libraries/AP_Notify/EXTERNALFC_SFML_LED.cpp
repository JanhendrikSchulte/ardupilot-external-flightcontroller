...
#include "EXTERNALFC_SFML_LED.h"

/*
  return layout size for a LED layout scheme
 */
bool SITL_SFML_LED::layout_size(EXTERNALFC::LedLayout layout, uint16_t &xsize, uint16_t &ysize)
{
    switch (layout) {
    case EXTERNALFC::LedLayout::ROWS:
        xsize = MAX_LEDS;
        ysize = 16;
        break;

    case EXTERNALFC::LedLayout::LUMINOUSBEE:
        xsize = 5 + 3 + 5;
        ysize = 5 + 3 + 5;
        break;
    default:
 ...
 */
bool SITL_SFML_LED::layout_pos(EXTERNALFC::LedLayout layout, uint8_t chan, uint8_t led, uint16_t &xpos, uint16_t &ypos)
{
    switch (layout) {
    case EXTERNALFC::LedLayout::ROWS:
        xpos = led;
...
void SITL_SFML_LED::update_serial_LEDs()
{
    ...
    EXTERNALFC::LedLayout layout = EXTERNALFC::LedLayout(sitl->led_layout.get());
    if (w == nullptr) {
        uint8_t max_leds = 0;
